#!/usr/bin/python3
# script to generate pdf using wkhtmltopdf and userpassword protect the pdf using qpdf
...
# $snappy = new Pdf();
# $snappy->setBinary('/usr/local/bin/wkhtmltopdfprotect.py userpassword ownerpassword');
# \PDF::setBinary('/usr/local/bin/wkhtmltopdfprotect.py userpassword ownerpassword')
import os
import sys
import shutil
...
ownerpassword=sys.argv[2]
arguments=sys.argv[3:] #get arguments for wkhtml
...
try:
    out_qpdf = Popen([qpdf_path,'--encrypt' ,userpassword ,ownerpassword ,'256', '--use-aes=y','--',backup_output_file,output_file], stdout=PIPE)
